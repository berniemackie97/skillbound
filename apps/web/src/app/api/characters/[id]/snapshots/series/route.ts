...
import { buildSnapshotSeries } from '@/lib/snapshots/snapshot-series';
...
export async function GET(
  ...
  const series = buildSnapshotSeries(snapshots, bucket);
  ...
  return NextResponse.json({
    data: series.map((point) => ({
      ...
      totalLevel: point.totalLevel,
      totalXp: point.totalXp,
      combatLevel: point.combatLevel,
    })),
    meta: {
      bucket,
      count: series.length,
