...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const id of ids) {
      const cacheKey = `integrations:osrsbox:monster:${id}`;
      if (cache) {
        const cached = await cache.get(cacheKey);
        if (cached) {
          cachedMap[String(id)] = true;
          monsters.push(cached);
          continue;
        }
      }

      const monster = await client.getMonster(id);
      cachedMap[String(id)] = false;
      monsters.push(monster);
      if (cache) {
        await cache.set(cacheKey, monster, cacheTtlMs);
      }
    }
    ...
    if (error instanceof OsrsBoxNotFoundError) {
      const problem = createProblemDetails({
        ...
        detail: `No OSRSBox monster found for id ${error.id}.`,
        instance: request.nextUrl.pathname,
      });
    ...
    console.error('OSRSBox monster lookup error:', error);
    const problem = createProblemDetails({
      status: 500,
