...
export async function GET(request: NextRequest) {
  ...
  const rateLimitId = `ge:5m:${getClientIp(request)}`;
  ...
  try {
    ...
    const result = await client.get5MinutePrices(parsed.data.timestamp);
    ...
    console.error('GE 5m price error:', error);
    const problem = createProblemDetails({
      status: 502,
      title: 'Upstream error',
      detail: 'Failed to fetch 5-minute prices.',
      instance: request.nextUrl.pathname,
    });
