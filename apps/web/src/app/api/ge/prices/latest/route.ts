...
export async function GET(request: NextRequest) {
  ...
  const rateLimitId = `ge:latest:${getClientIp(request)}`;
  ...
  try {
    ...
    const cacheTtlMs = getWikiCacheTtlMs('latest');
    const userAgent =
    ...
    const result = await client.getLatestPrices(parsed.data.id);
    ...
    console.error('GE latest price error:', error);
    const problem = createProblemDetails({
      status: 502,
      title: 'Upstream error',
      detail: 'Failed to fetch latest prices.',
      instance: request.nextUrl.pathname,
    });
